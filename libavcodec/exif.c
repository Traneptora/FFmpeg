                               ...
                               int depth, AVExifMetadata *ifd)
{
    ...
    if (bytestream2_get_bytes_left(gb) < entries * 12) {
        av_log(logctx, AV_LOG_ERROR, "not enough bytes remaining in EXIF buffer\n");
        return AVERROR_INVALIDDATA;
    }

    ifd->count = entries;
    /*
     * we allocate space for eight more entries than we need
     * This way if we later add more our calls to av_realloc will no-op
     */
    ifd->entries = av_calloc(FFMAX(entries + 8, 0xFFFFu), sizeof(*ifd->entries));
    if (!ifd->entries)
        return AVERROR(ENOMEM);
...
static void exif_free_entry(AVExifEntry *entry)
{
    ...
    /*
     * we zero this here even though it's already been freed
     * because we also call this function with av_exif_clone_ifd
     */
...
static size_t exif_get_ifd_size(const AVExifMetadata *ifd)
{
    ...
    for (size_t i = 0; i < ifd->count; i++) {
        ...
        if (entry->type == AV_TIFF_IFD) {
            ...
            total_size += 12 + (payload_size > 4 ? payload_size : 0);
        }
    }
    return total_size;
}
...
static int exif_write_ifd(void *logctx, PutByteContext *pb, int le, int depth, const AVExifMetadata *ifd)
{
    ...
    av_log(logctx, AV_LOG_DEBUG, "writing IFD with %zu entries and initial offset %d\n", ifd->count, offset);
    for (size_t i = 0; i < ifd->count; i++) {
        const AVExifEntry *entry = &ifd->entries[i];
...
int av_exif_write(void *logctx, const AVExifMetadata *ifd, AVBufferRef **buffer, enum AVExifHeaderMode header_mode)
{
    ...
    buf = av_buffer_alloc(size + off + 4);
    if (!buf)
        return AVERROR(ENOMEM);
    ...
    tput16(&pb, le, le ? 0x4949u : 0x4d4du);
    tput16(&pb, le, 42);
    tput32(&pb, le, 8);
...
#define EXIF_COPY(fname, srcname) do { \
    size_t sz = src->count * sizeof(*(fname)); \
    if (src->count && sz / src->count != sizeof(*(fname))) { \
        ret = AVERROR(ENOMEM); \
        goto end; \
    } \
    (fname) = av_malloc(sz); \
...
static int exif_clone_entry(AVExifEntry *dst, const AVExifEntry *src)
{
    ...
    if (src->ifd_lead) {
        dst->ifd_lead = av_malloc(src->ifd_offset);
        if (!dst->ifd_lead) {
            ret = AVERROR(ENOMEM);
            goto end;
        }
        dst->ifd_offset = src->ifd_offset;
        memcpy(dst->ifd_lead, src->ifd_lead, src->ifd_offset);
    }
    ...
    uint32_t count, const uint8_t *ifd_lead, uint32_t ifd_offset, const void *value)
{
    ...
    if (entry) {
        ...
        temp = av_realloc_array(ifd->entries, FFMAX(ifd->count + 1, 8), sizeof(*ifd->entries));
        if (!temp)
            return AVERROR(ENOMEM);
        ifd->entries = temp;
...
AVExifMetadata *av_exif_clone_ifd(const AVExifMetadata *ifd)
{
    ...
    ret->count = ifd->count;
    if (ret->count) {
        ret->entries = av_calloc(ret->count, sizeof(*ret->entries));
        if (!ret->entries)
            goto fail;
    }
