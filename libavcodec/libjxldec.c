...
static int libjxl_init_jxl_decoder(AVCodecContext *avctx)
{
    ...
    if (JxlDecoderSetDecompressBoxes(ctx->decoder, JXL_TRUE) != JXL_DEC_SUCCESS) {
        av_log(avctx, AV_LOG_ERROR, "Error setting compresss box mode\n");
        return AVERROR_EXTERNAL;
    }
    ...
    while (1) {
        ...
        case JXL_DEC_FULL_IMAGE:
            ...
            if (ctx->exif) {
                ...
                // won't overflow, min size 4k
                uint32_t tiff_header_off = AV_RB32(ctx->exif->data) + 4;
                if (tiff_header_off > ctx->exif_pos) {
                    av_log(avctx, AV_LOG_ERROR, "invalid Exif box TIFF header offset\n");
                    av_buffer_unref(&ctx->exif);
                    continue;
                }
                /* size may be larger than exif_pos due to the realloc loop */
                ret = av_exif_parse_buffer(avctx, ctx->exif->data + tiff_header_off, ctx->exif_pos - tiff_header_off,
                                           &ifd, AV_EXIF_PARSE_TIFF_HEADER);
                av_buffer_unref(&ctx->exif);
                if (ret < 0) {
        ...
        case JXL_DEC_BOX_NEED_MORE_OUTPUT: {
            ...
            size_t count = JxlDecoderReleaseBoxBuffer(ctx->decoder);
        ...
        case JXL_DEC_BOX_COMPLETE: {
            ...
            ctx->exif_pos += JxlDecoderReleaseBoxBuffer(ctx->decoder);
            continue;
        }
        default:
